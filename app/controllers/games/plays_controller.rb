...
class Games::PlaysController < ApplicationController
  ...
  def command
    ...
    respond_to do |format|
      format.turbo_stream { redirect_to game_play_path(@game), alert: e.message }
  ...
  def undo
    ...
    respond_to do |format|
      format.turbo_stream { redirect_to game_play_path(@game), alert: 'ゲームの復元に失敗しました。時間をおいて再度お試しください。' }
